...
def main():
    print("Mencari KOL melalui Google Search (Apify)...")
    client = ApifyClient(APIFY_API_TOKEN)
    
    queries = [
        'site:instagram.com "influencermalang" OR "standupcomedymalang" OR "malangcontentcreator"',
        'site:instagram.com/p/ "influencermalang" OR "standupcomedymalang" OR "malangcontentcreator"',
        'site:instagram.com/reel/ "influencermalang" OR "standupcomedymalang" OR "malangcontentcreator"'
    ]
    ...
    for item in items:
        for res in item.get('organicResults', []):
            ...
            # 2. Coba ambil dari Title (biasanya format "Nama (@username) • Instagram...")
    ...
    if usernames:
        print("Memulai scraping profil dari username yang ditemukan...")
        ...
        print(f"Ada {len(new_usernames)} profil yang benar-benar baru dan belum ada di database.")
        
        if new_usernames:
            # Batasi misal 50 max
            new_usernames = new_usernames[:50]
            results = scraper.scrape_profiles_parallel(new_usernames)
            if results:
                print(f"Berhasil menambahkan {len(results)} profil baru ke database!")
            else:
                print("Tidak ada profil baru yang berhasil didapat (mungkin karena filter area Malang).")
        else:
            print("Semua profil sudah ada di database.")
